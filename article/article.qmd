...
TODO uitleggen dat het niet per se tegengestelden zijn: elk verklaren ze iets anders
...
### Language change {#sec-review-language-change}
...
In our study, then, we aim to investigate to what extent priming, when modelled according to the two-stage competition model [@segaert_unifying_2016], has an influence on language change, which we define as a change in the base rate of an innovative construction. Our model should thus explicitly support both cumulativity and the inverse frequency effect [see @sec-review-priming-properties]. In order to not complicate the design of our study, we decided not to focus on the lexical boost effect, especially since it is moreso an additional effect. In the next section, we will detail the design of our simulation model, and how exactly we implemented the two-stage competition model.
...
First and foremost, since replicator selection (see @sec-review-language-change) is *always* assumed to be at play, the innovative construction will always become dominant over time. The influence of priming can be estimated by comparing the evolution of the base rate across agents in simulations *with* priming and *without* priming. From @dekker_conversational_2024, priming is expected to accelerate the dominance of the innovative construction.

Additionally, however, we can check the influence of the inclusion of the inverse frequency effect by again comparing the evolution of the base rate across agents in simulations *without* inverse frequency and *with* inverse frequency. Especially the interaction with replicator selection is interesting: do we see a further acceleration, or other, unpredictable behaviour? Depending on the outcome of these simulations, we can incrementally compare a baseline with a new situation to better understand the possible contribution of each mechanism to the propagation of an innovative construction, and more broadly, also language change. Usually in computer simulations, the principle of Occam's razor is applied [@pijpops_agent-gebaseerde_2015, 12], which implies that mechanisms are incrementally removed from a simulation to check for their influence. Since we start from a situation without priming and comparatively check the influence of each newly added mechanism, we are effectively applying this principle in reverse.
...
In this section, we show the results of the different parameter permutations in our computer simulations. Each subsection investigates the influence of one specific mechanism, in accordance with our prescribed way of analysis defined in @sec-evaluation-priming-influence.

### Influence of priming, no inverse frequency effect



### Influence of priming, inverse frequency effect

### Influence of priming, influence of construction frequency

ook aandacht hebben voor cumulativity! bij frequentie

### Influence of priming, influence of replicator selection strength
